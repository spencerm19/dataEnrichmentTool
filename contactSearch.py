...
def contact_search(input_filename, jwt_token, last_auth_time, username, password):
    ...
    for entry in data:
        ...
        if entry.get('needsContact') == 'Yes':
            ...
            if entry.get('zi_c_location_id'):
                person_id = get_contact_person_id(entry, jwt_token, strict=True, use_location_id=True)
                if person_id:
                    entry['personId'] = person_id
                    entry['contactMatchCriteria'] = 'locationId_strict'
                    person_id_count += 1
                else:
                    person_id = get_contact_person_id(entry, jwt_token, strict=False, use_location_id=True)
                    if person_id:
                        entry['personId'] = person_id
            ...
            if person_id is None and entry.get('zi_c_company_id'):
                person_id = get_contact_person_id(entry, jwt_token, strict=True, use_location_id=False)
                if person_id:
                    entry['personId'] = person_id
                    entry['contactMatchCriteria'] = 'companyId_strict'
                    person_id_count += 1
                else:
                    person_id = get_contact_person_id(entry, jwt_token, strict=False, use_location_id=False)
                    if person_id:
                        entry['personId'] = person_id
